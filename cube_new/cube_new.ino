...
void loop() {
  ...
  if(central) {
   //delay(5000);
    //Serial.print(StateService.uuid());
    //delay(10000);
    if(synTime) {
      // Запрашиваем время.
      // ...
  ...
  for (int i = 0; i < 12; i++) {
    if (state[i] != diodeOn[i]) {
      if (state[i]) {
        ...
        if (i < 6) analogWrite(diodePin[i], BRIGHTNESS);
        else digitalWrite(diodePin[i], HIGH);
      } else {
        Serial.print(F("Diode #"));
        Serial.print(i);
        Serial.println(" is off");
        if (i < 6) analogWrite(diodePin[i], 0);
        else digitalWrite(diodePin[i], LOW);
      }
      diodeOn[i] = state[i];
    }
  }*/
